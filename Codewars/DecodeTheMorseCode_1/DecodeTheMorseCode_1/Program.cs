...
namespace DecodeTheMorseCode_1
{
    class Program
    {
    ...
    class MorseCodeDecoder
    {
        public static string Decode(string morseCode)
        {
            ...
            {
                string[] arrTemp = Regex.Split(word, letterSeparator);
                foreach (string encodedWord in arrTemp)
                {
                    result.Append(MorseCode.Get(encodedWord));
                }
                result.Append(" ");
            }
            string response = result.ToString().TrimEnd();
            return response;
        }
    }
